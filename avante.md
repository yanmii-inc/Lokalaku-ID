# Lokalaku AI Agent Instructions

This file provides project-specific context and instructions for AI assistants working in the Lokalaku codebase via avante.nvim.

## Quick Start

Before making changes, AI agents should read the relevant documentation based on the task complexity:

### Task Tiers (Read Only What's Needed)

**Tier 0** — No documentation needed:
- Simple fixes: typos, formatting, import ordering, linter errors
- Isolated UI tweaks: colors, spacing
- Adding null checks or guard clauses
- Local variable renaming

**Tier 1** — Read one AGENTS.md file:
- Features or fixes within a single app/package
- App-specific rules and conventions
...
- New API endpoints, DB tables, Flutter screens
...
- Modifying core state machines, auth flows, sync strategies
- Adding domain entities or shared packages

Read:
- `PRD.md` (root) — full product scope
- `docs/decisions/ADR-*.md` — existing architectural decisions

**Tier 4** — Understanding historical context:
- "Why did we choose X over Y?"
- Reviewing milestone history

Read:
- `docs/decisions/ADR-*.md` — specific ADR
- `docs/milestones/MXXX-*.md` — milestone documentation

## Core Architectural Principles

1. **Zero Vendor Lock-In**: Deployable on bare-metal VPS or Docker. No cloud-native proprietary APIs (Vercel, AWS-only tools).

2. **Offline-First Operations**: Mobile apps (Merchant/Courier) must handle low/no internet via local caching.
...
This project includes AI agent skills for common workflows:
...
Skills are located in `.agents/skills/` and can be invoked by name.